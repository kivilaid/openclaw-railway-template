...
function pushLog(text) {
  ...
  for (const line of lines) {
    liteLogBuffer.push({ ts, text: line });
    if (liteLogBuffer.length > LITE_LOG_BUFFER_MAX) liteLogBuffer.shift();
  }
}

// Intercept console.log/error to capture logs for the /lite dashboard viewer
...
app.get("/lite/api/status", requireSetupAuth, async (_req, res) => {
  const running = isGatewayReady();
  let model = null;
  let channels = [];
  let uptime = null;

  if (running && gatewayStartedAt) {
    uptime = Math.floor((Date.now() - gatewayStartedAt) / 1000);
  }

  // Try to get model from config
  try {
    const r = await runCmd(OPENCLAW_NODE, clawArgs(["config", "get", "models.default"]));
    if (r.code === 0 && r.output.trim()) model = r.output.trim();
  } catch {}
  ...
  res.json({ running, uptime, model, channels });
});

app.get("/lite/api/logs", requireSetupAuth, (req, res) => {
  const since = Number(req.query.since) || 0;
  const lines = since ? liteLogBuffer.filter(l => l.ts > since) : liteLogBuffer.slice(-200);
  res.json({ lines });
});

app.post("/lite/api/gateway/start", requireSetupAuth, async (_req, res) => {
  try {
    await ensureGatewayRunning();
...
app.post("/lite/api/gateway/stop", requireSetupAuth, (_req, res) => {
  if (gatewayProc) {
...
app.post("/lite/api/gateway/restart", requireSetupAuth, async (_req, res) => {
  try {
    await restartGateway();
    gatewayStartedAt = Date.now();
    res.json({ ok: true });
...
app.get("/lite/api/stats", requireSetupAuth, async (_req, res) => {
  let skills = 0;
  let sessions = 0;
  let version = null;
  ...
  try {
    ...
    if (fs.existsSync(skillsDir)) {
      skills = fs.readdirSync(skillsDir).filter(f => !f.startsWith(".")).length;
    }
  } catch {}

  // Also check workspace skills
  try {
    const wsSkills = path.join(WORKSPACE_DIR, "skills");
    if (fs.existsSync(wsSkills)) {
      skills += fs.readdirSync(wsSkills).filter(f => !f.startsWith(".")).length;
    }
  } catch {}

  // Count sessions
  try {
    const sessDir = path.join(STATE_DIR, "sessions");
    if (fs.existsSync(sessDir)) {
      sessions = fs.readdirSync(sessDir).filter(f => !f.startsWith(".")).length;
    }
  } catch {}
  ...
  try {
    ...
    if (r.code === 0) version = r.output.trim();
  } catch {}

  res.json({ skills, sessions, version });
});

app.get("/lite/api/usage", requireSetupAuth, async (_req, res) => {
  // Best effort: try to parse usage from openclaw CLI or return empty
  try {
    const r = await runCmd(OPENCLAW_NODE, clawArgs(["usage", "--json", "--days", "7"]));
    if (r.code === 0 && r.output.trim()) {
      const data = JSON.parse(r.output.trim());
      return res.json({ days: data.days || data || [] });
    }
  } catch {}
  // Fallback: empty
  res.json({ days: [] });
...
app.get("/lite/api/memory", requireSetupAuth, (_req, res) => {
  let backend = "file";
  let entries = 0;
  let files = 0;
  ...
  try {
    ...
    if (fs.existsSync(memDir)) {
      const allFiles = fs.readdirSync(memDir).filter(f => f.endsWith(".md"));
      files = allFiles.length;
      // Count approximate entries (lines with content)
  ...
  res.json({ backend, entries, files });
});

app.get("/lite/api/memory/search", requireSetupAuth, (req, res) => {
        ...
        try {
          ...
          for (let i = 0; i < lines.length; i++) {
            if (lines[i].toLowerCase().includes(q)) {
              // Get surrounding context (1 line before/after)
  ...
  try {
    const rootMds = fs.readdirSync(WORKSPACE_DIR).filter(f => f.endsWith(".md"));
    for (const f of rootMds) {
      try {
        const content = fs.readFileSync(path.join(WORKSPACE_DIR, f), "utf8");
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
          if (lines[i].toLowerCase().includes(q)) {
            const start = Math.max(0, i - 1);
...
app.post("/lite/api/memory/index", requireSetupAuth, async (_req, res) => {
  try {
    const r = await runCmd(OPENCLAW_NODE, clawArgs(["memory", "index"]));
...
app.get("/lite/api/cron", requireSetupAuth, async (_req, res) => {
  try {
    const r = await runCmd(OPENCLAW_NODE, clawArgs(["cron", "list", "--json"]));
    if (r.code === 0 && r.output.trim()) {
      try {
        const data = JSON.parse(r.output.trim());
        return res.json({ tasks: Array.isArray(data) ? data : data.tasks || [] });
      } catch {
        // Parse text output
        const tasks = r.output.trim().split("\n").filter(Boolean).map(l => ({ command: l, schedule: "", status: "" }));
        return res.json({ tasks });
      }
    }
    // Try without --json
    const r2 = await runCmd(OPENCLAW_NODE, clawArgs(["cron", "list"]));
    if (r2.code === 0 && r2.output.trim()) {
      const tasks = r2.output.trim().split("\n").filter(Boolean).map(l => ({ command: l, schedule: "", status: "" }));
      return res.json({ tasks });
    }
  } catch {}
  res.json({ tasks: [] });
...
app.post("/lite/api/security-audit", requireSetupAuth, async (_req, res) => {
  try {
...
app.get("/lite/api/version", requireSetupAuth, async (_req, res) => {
  let current = null;
  let latest = null;

  try {
    const r = await runCmd(OPENCLAW_NODE, clawArgs(["--version"]));
    if (r.code === 0) current = r.output.trim();
  } catch {}
...
app.post("/lite/api/upgrade", requireSetupAuth, async (_req, res) => {
  try {
    const r = await runCmd("npm", ["install", "-g", "openclaw@latest"]);
    cachedOpenclawVersion = null; // Bust cache
    res.json({ ok: r.code === 0, output: r.output });
  } catch (err) {
    res.json({ ok: false, error: err.message });
  }
...
app.get("/lite/api/backup", requireSetupAuth, async (_req, res) => {
  fs.mkdirSync(STATE_DIR, { recursive: true });
  fs.mkdirSync(WORKSPACE_DIR, { recursive: true });
  ...
  tar.stderr.on("data", (d) => console.warn("[lite/backup] tar stderr:", d.toString()));
  tar.on("error", (err) => {
    console.error("[lite/backup] tar error:", err);
    if (!res.headersSent) res.status(500).end();
  });
...
// Restore from uploaded .tar.gz or .zip (multipart)
app.post("/lite/api/restore", requireSetupAuth, (req, res) => {
  // Manually parse multipart boundary to extract file data without external deps
  ...
  req.on("end", async () => {
    if (totalSize > MAX_SIZE) {
      return res.status(413).json({ ok: false, error: "File too large (max 250MB)" });
    }
    ...
    try {
      ...
      // Find start of file data (after headers in the first part)
      const boundaryBuf = Buffer.from("--" + boundary);
      let start = body.indexOf(boundaryBuf);
      if (start === -1) return res.status(400).json({ ok: false, error: "Invalid multipart data" });
      ...
      const fileData = body.subarray(start, end);
      const tmpPath = path.join(os.tmpdir(), `openclaw-restore-${Date.now()}.tar.gz`);
      fs.writeFileSync(tmpPath, fileData);

      const stateAbs = path.resolve(STATE_DIR);
      const dataRoot = "/data";
      const restoreDir = stateAbs.startsWith(dataRoot) ? dataRoot : "/";

      const tarResult = await runCmd("tar", ["-xzf", tmpPath, "-C", restoreDir]);
      fs.rmSync(tmpPath, { force: true });

      if (tarResult.code !== 0) {
        return res.status(500).json({ ok: false, error: "Restore failed: " + tarResult.output });
      }

      res.json({ ok: true, output: "Restore complete." });
    } catch (err) {
      res.status(500).json({ ok: false, error: err.message });
    }
  });
...
proxy.on("proxyReq", (proxyReq, req, res) => {
  // For /hooks/* paths, preserve the original Authorization header (webhook callers
  // send the hooks.token there) and pass the gateway token via x-openclaw-gateway-token
  // so the gateway can still authenticate the connection.
  const hooksPath = (process.env.OPENCLAW_HOOKS_PATH || "/hooks").replace(/\/$/, "");
  if (req.url === hooksPath || req.url.startsWith(hooksPath + "/") || req.url.startsWith(hooksPath + "?")) {
    proxyReq.setHeader("x-openclaw-gateway-token", OPENCLAW_GATEWAY_TOKEN);
  } else {
...
const liteWss = new WebSocketServer({ noServer: true });

// /lite/ws — interactive bash shell (same pattern as TUI but spawns bash)
let activeLiteSession = null;

liteWss.on("connection", (ws, req) => {
  const clientIp = req.socket?.remoteAddress || "unknown";
  console.log(`[lite/ws] terminal session started from ${clientIp}`);

  let ptyProcess = null;

  activeLiteSession = { ws, pty: null, startedAt: Date.now() };
  ...
  function spawnPty(cols, rows) {
    ...
    if (activeLiteSession) activeLiteSession.pty = ptyProcess;
    ...
    ptyProcess.onExit(({ exitCode, signal }) => {
      console.log(`[lite/ws] PTY exited code=${exitCode} signal=${signal}`);
  ...
  ws.on("close", () => {
    console.log("[lite/ws] session closed");
    if (ptyProcess) { try { ptyProcess.kill(); } catch {} }
    activeLiteSession = null;
  });

  ws.on("error", (err) => console.error(`[lite/ws] error: ${err.message}`));
});

server.on("upgrade", async (req, socket, head) => {
  const url = new URL(req.url, `http://${req.headers.host}`);

  if (url.pathname === "/lite/ws") {
    if (!verifyTuiAuth(req)) {
      socket.write("HTTP/1.1 401 Unauthorized\r\n\r\n");
      socket.destroy();
      return;
    }
    if (activeLiteSession) {
      socket.write("HTTP/1.1 409 Conflict\r\n\r\n");
      socket.destroy();
      return;
    }
    liteWss.handleUpgrade(req, socket, head, (ws) => {
      liteWss.emit("connection", ws, req);
    });
    return;
  }
...
async function gracefulShutdown(signal) {
  ...
  if (activeLiteSession) {
    try {
      activeLiteSession.ws.close(1001, "Server shutting down");
      if (activeLiteSession.pty) activeLiteSession.pty.kill();
    } catch {}
    activeLiteSession = null;
  }
